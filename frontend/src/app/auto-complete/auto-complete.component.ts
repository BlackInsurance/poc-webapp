...
export class AutoCompleteComponent implements OnInit, OnChanges {
  ...
  private setUserSettings(): Settings {
    ...
    if (this.userSettings && typeof(this.userSettings) === 'object') {
      ...
      let returnSettings = this.defaultSettings;
      let configuredSettings = this._elmRef.nativeElement.attributes;
      let settingsKeys = Object.keys(returnSettings);

      settingsKeys.forEach((v,i,s)=>{
        try{
          let currentConfiguredSetting = configuredSettings.getNamedItem(v);
          if ( currentConfiguredSetting ){
            returnSettings[v] = currentConfiguredSetting.value;
          }
        }catch(err){}
      });
